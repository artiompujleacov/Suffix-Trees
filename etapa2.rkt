...
(define (ast-func suffixes)
  (let ((label (list (caar suffixes))) (new-suffixes (map cdr suffixes)))
...
(define (cst-func suffixes)
  (define (common-prefix list1 list2)
    (let loop ((l1 list1) (l2 list2) (prefix '()))
      (if (or (null? l1) (null? l2) (not (eq? (car l1) (car l2))))
          (reverse prefix)
          (loop (cdr l1) (cdr l2) (cons (car l1) prefix)))))
...
(define (suffixes->st labeling-func suffixes alphabet)
  ...
  (filter (compose not null?) (map (lambda (ch) (build-branch ch)) alphabet)))
